...
const EnvConfigPath = "MP_ARTICLE_DOWNLOADER_CONFIG_FILEPATH"
...
func LoadCertFiles() (*certificate.CertFileAndKeyFile, error) {
	...
	var certDir string
	if home, err := os.UserHomeDir(); err == nil {
		certDir = filepath.Join(home, ".config", "mp-article-batch-downloader", "certs")
	} else {
		certDir = filepath.Join(os.TempDir(), "mp-article-batch-downloader", "certs")
	}
	return certificate.LoadOrCreateDefault(certDir)
}
