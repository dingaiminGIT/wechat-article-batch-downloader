...
func NewAPIClient(cfg *APIConfig, parent_logger *zerolog.Logger) *APIClient {
	...
	officialaccountdownload.ArticleAuthProvider = func(biz string) officialaccountdownload.ArticleAuthCredential {
		cred := officialaccount_client.ArticleAuthCredential(biz)
		return officialaccountdownload.ArticleAuthCredential{
			Uin:        cred.Uin,
			Key:        cred.Key,
			PassTicket: cred.PassTicket,
			Cookie:     cred.Cookie,
		}
	}
