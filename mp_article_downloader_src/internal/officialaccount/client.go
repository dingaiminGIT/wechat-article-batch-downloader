...
func (c *OfficialAccountClient) ArticleAuthCredential(biz string) ArticleAuthCredential {
	acct_mu.RLock()
	defer acct_mu.RUnlock()
	acct := accounts[biz]
	if acct == nil {
		return ArticleAuthCredential{}
	}
	cookie := acct.Cookie
	if acct.CookieExpiration != 0 && time.Now().Unix() >= acct.CookieExpiration {
		cookie = ""
	}
	return ArticleAuthCredential{
		Uin:        acct.Uin,
		Key:        acct.Key,
		PassTicket: acct.PassTicket,
		Cookie:     cookie,
	}
}
...
func (c *OfficialAccountClient) HandleOfficialAccountRSS(ctx *gin.Context) {
	...
	atom := AtomFeed{
		...
		Generator: "mp_article_batch_downloader",
		Icon:      acct.AvatarURL,
...
func save_accounts() {
	...
	err = os.WriteFile(mp_json_filepath, data, 0600)
	if err != nil {
...
func load_accounts() {
	...
	_ = os.Chmod(mp_json_filepath, 0600)
