...
build_windows() {
    ...
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -trimpath -tags with_gvisor -ldflags="-s -w" -o "$OUTPUT_DIR/mp_article_batch_downloader_windows_x86_64.exe"
    echo "Done: $OUTPUT_DIR/mp_article_batch_downloader_windows_x86_64.exe"
}
...
build_windows_sunnynet() {
    ...
    docker run --rm \
        ...
        bash -c '
            ...
            -o mp_article_batch_downloader_sunnynet.exe .
        '
    echo "Done: mp_article_batch_downloader_sunnynet.exe"
}

build_macos() {
    echo "Building macOS..."
    CGO_ENABLED=1 GOOS=darwin SDKROOT=$(xcrun --sdk macosx --show-sdk-path) go build -trimpath -ldflags="-s -w" -o "$OUTPUT_DIR/mp_article_batch_downloader_macos"
    echo "Done: $OUTPUT_DIR/mp_article_batch_downloader_macos"
}

build_macos_arm64() {
    echo "Building macOS arm64..."
    CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 SDKROOT=$(xcrun --sdk macosx --show-sdk-path) go build -trimpath -ldflags="-s -w" -o "$OUTPUT_DIR/mp_article_batch_downloader_macos_arm64"
    echo "Done: $OUTPUT_DIR/mp_article_batch_downloader_macos_arm64"
}

build_linux() {
    echo "Building Linux x86_64..."
    CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w" -o "$OUTPUT_DIR/mp_article_batch_downloader_linux"
    echo "Done: $OUTPUT_DIR/mp_article_batch_downloader_linux"
}

build_linux_arm64() {
    echo "Building Linux arm64..."
    CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -trimpath -ldflags="-s -w" -o "$OUTPUT_DIR/mp_article_batch_downloader_linux_arm64"
    echo "Done: $OUTPUT_DIR/mp_article_batch_downloader_linux_arm64"
}
